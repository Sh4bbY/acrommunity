<template>
  <v-card class="mb-5">
    <media-dialog v-model="dialog.show" :item="dialog.item" type="video" :fullscreen="$vuetify.breakpoint.xs" :is-first="dialog.isFirst" :is-last="dialog.isLast" @next="loadNext"
                  @prev="loadPrev"/>
    ...
    <v-card-text>
      <vue-horizontal responsive v-if="videos.length > 0">
        <div v-for="(item,i) in videos" :key="i" class="item-container" @click="showDialog(item)">
