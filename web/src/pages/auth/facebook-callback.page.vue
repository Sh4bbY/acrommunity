...
export default class FacebookAuthCallbackPage extends Page {
  async mounted() {
    try {
      await this.$store.state.auth.authPendingPromise;
      if (!this.$store.state.auth.isSignedIn && this.$route.query.code) {
        await this.$store.dispatch('auth/handleFacebookAuthCallback', this.$route.query);
      } else {
